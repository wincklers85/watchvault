...
export async function fetchWatchLifecycle(watchId:string){const [events,transfers,listings]=await Promise.all([
 rest(`watch_passport_events?watch_id=eq.${watchId}&select=*&order=event_date.desc,created_at.desc`).then(r=>r.json()),
 rest(`watch_transfers?watch_id=eq.${watchId}&select=*&order=created_at.desc`).then(r=>r.json()),
 rest(`marketplace_listings?watch_id=eq.${watchId}&select=id,status,price,currency,created_at,sold_at,buyer_id&order=created_at.desc`).then(r=>r.json())
]);return{events,transfers,listings}}
